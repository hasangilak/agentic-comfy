...
Each beat's opening still can be **generated by agy or uploaded** — drag an image onto the
frame, or click ⤒ in the node's header. Uploading costs no quota, which matters because
image generation is capped at roughly five per five hours. Supplying a still makes that beat
a new shot, so its wire switches to a cut; anything far from 9:16 gets a crop warning before
you pay to discover it.
